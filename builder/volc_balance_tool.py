...
# ===================== 获取程序（exe/脚本）所在目录 =====================
def get_exe_dir():
    """获取程序所在目录，打包后返回exe目录，开发环境返回脚本目录"""
...
class VolcBalanceConfigTool:
  ...
  extractor: function(response) {
    ...
    def generate_cc_js_file(self):
        # 不再跟着Jar包路径走，直接保存到exe所在目录
        js_path = os.path.join(get_exe_dir(), "cc-switch-balance-config.js")
    ...
    def get_yml_path(self):
        # 不再跟着Jar包路径走，直接保存到exe所在目录
        return os.path.join(get_exe_dir(), "application.yml")
    ...
    def load_config_from_yml(self):
        try:
            # 不再从当前工作目录加载，直接从exe所在目录找
            yml_path = os.path.join(get_exe_dir(), "application.yml")
